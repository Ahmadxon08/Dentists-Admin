...
const QarzTableComponent = () => {
  ...
  const paidAppointments = appointments.filter((a) => (a.givenPrice ?? 0) > 0);

  const filteredAppointments = paidAppointments.filter(
    (appt) =>
      appt.firstName.toLocaleLowerCase().includes(searchQuery.toLowerCase()) ||
      appt.lastName.toLocaleLowerCase().includes(searchQuery.toLowerCase()) ||
      appt.doctor.toLocaleLowerCase().includes(searchQuery.toLocaleLowerCase())
  );
  ...
  }, [filteredAppointments, totalPages, currentPage]);
  ...
  return (
    <>
      ...
      {searchQuery.length > 0 && filteredAppointments.length === 0 ? (
        ...
        <div className="text-center flex flex-col h-fit w-full text-gray-500">
          <h1 className="font-semibold text-[22px]">Qarzdorlar yo&#39;q</h1>
          <p>Ushbu holatda qarzdorlar topilmadi</p>
        </div>
      )}
    </>
  );
...
export default QarzTableComponent;
